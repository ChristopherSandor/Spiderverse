...
public class TrackSpot {
    ...
    public TrackSpot(Collider adjencyList){
    ...
    public static void main(String[] args) {
        ...
        Collider adjencyList = new Collider(5);
        adjencyList.createCollider(args[0], args[1]);

        Trackspot tracker = new TrackSpot(adjencyList);
        Dimension starting;
        Dimension ending;
    ...
    public ArrayList<Dimension> findPath (Dimension startingDimension, Dimension endDimension){

        // Indexs will be the same comparied to collider object
        
        boolean[] marked = new boolean[adjencyList.getList().length];
    ...
    private boolean[] mark(boolean[] array, int dimenisionNumber){
        int index = adjencyList.getPositionIndex(dimenisionNumber);
        array[index] = true;
